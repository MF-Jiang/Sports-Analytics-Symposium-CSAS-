...
def process_group(group):
    return add_previous_pitches(group)

def parallelize_dataframe(df, func, num_workers=4):
    groups = [group for _, group in df.groupby(['pitcher', 'game_pk'])]
    with Pool(num_workers) as pool:
        result = pool.map(func, groups)
    return pd.concat(result)

if __name__ == '__main__':
    data=pd.read_csv('../data/../data/pitcher_prediction_dataset/pitcher_prediction_dataset_V3.csv', low_memory=False)
    ...
    data_parallelized.to_csv("../data/pitcher_prediction_dataset/pitcher_prediction_dataset_V4.csv", index=False)
    print("save v3")
